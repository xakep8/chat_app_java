...
export default function Messages() {
  ...
  const { messages, setMessages, sendMessage, isConnected } = useChat(selectedChatId);
    ...
    <div className="flex h-screen w-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className="w-[80px] bg-slate-900/50 backdrop-blur-xl border-r border-white/5 flex flex-col items-center py-8 gap-8 transition-all duration-300">
        ...
        <nav className="flex flex-col gap-4 flex-1">
          <button className="p-3 rounded-xl bg-indigo-500/10 text-indigo-400 group relative">
              ...
              return (
                <div key={msg.id || idx} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                  ...
                  {idx === messages.length - 1 && (
                    <span className="text-[9px] text-slate-600 mt-1 mr-1">{formatTime(msg.createdAt)}</span>
                  )}
                </div>
              );
            })
